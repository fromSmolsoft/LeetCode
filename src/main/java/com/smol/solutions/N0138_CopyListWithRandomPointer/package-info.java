 ...
 * Medium
 * <p>
 * A linked list of length n is given such that each node contains an additional random pointer, which could point to any node in the list, or null. <p>
 ...
 * For example, if there are two nodes X and Y in the original list, where X.random --> Y, then for the corresponding two nodes x and y in the copied list, x.random --> y. <p>
 * Return the head of the copied linked list.<p>
 ...
 * -  val: an integer representing Node.val <p>
